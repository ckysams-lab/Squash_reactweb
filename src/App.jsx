...
// --- 版本控制 ---
// Version 5.2: 獎項優化 (照片/Grid)
// Version 5.3: [Current] 導入 Firebase Auth (Email/Pass) 並實作身份綁定
...
export default function App() {
  ...
  const [systemConfig, setSystemConfig] = useState({ 
    adminPassword: 'admin', // Deprecated
    adminEmail: 'coach@bcklas.com', // [New in V5.3] 教練登入 Email
    announcements: [],
    ...
    try {
      ...
      const unsubSystemConfig = onSnapshot(systemConfigRef, (docSnap) => {
        if (docSnap.exists()) setSystemConfig(docSnap.data());
            ...
            adminPassword: 'admin', 
            adminEmail: 'coach@bcklas.com', // Default admin email
            announcements: [], 
            seasonalTheme: 'default', 
            schoolLogo: null 
        });
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (loginTab === 'admin') {
         // 教練登入檢查
         // 注意：為了更安全，這裡應該比對 systemConfig.adminEmail
         // 但由於 systemConfig 是非同步載入，第一次登入可能還沒抓到
         // 這裡我們先做一個簡單的假設：如果 Email 包含 "coach" 或 "admin" 或者是設定的 Email，則為 Admin
         // 更好的做法是：登入後，等待 systemConfig 載入再跳轉。
         
         // 這裡我們先直接允許進入，然後由 UI 顯示內容 (Firestore Rules 會在後端擋住非授權寫入)
         // 但為了前端體驗，我們比對一下
         ...
         if (loginEmail.toLowerCase() === (systemConfig?.adminEmail || 'coach@bcklas.com').toLowerCase()) {
             ...
             // 如果不是教練 Email，可能是學生誤按，或是新的教練。
             // 暫時先當作是教練進入，但實際上如果沒有 write 權限，他也做不了壞事
             // 為了方便，我們提示一下
             if (confirm("您輸入的 Email 與系統設定的教練 Email 不符。是否仍要以管理員身份進入？\n(若非教練請按取消)")) {
                 setRole('admin');
                 setShowLoginModal(false);
                 setActiveTab('dashboard');
             } else {
        ...
        // 學生登入
        // 我們需要去 students collection 找有沒有這個 email
        // 因為此時 students state 可能還沒載入 (剛登入)，所以我們直接 query 一次
        ...
        if (!querySnapshot.empty) {
            ...
            setCurrentUserInfo({ ...studentDoc, id: querySnapshot.docs[0].id });
            setRole('student');
            setShowLoginModal(false);
            setActiveTab('competitions');
        } else {
            // 找不到資料，可能是新學生
            alert("登入成功，但在隊員檔案庫中找不到此 Email。\n請聯繫教練將您的 Email 加入檔案庫。");
            // 暫時允許進入，顯示為訪客
            setCurrentUserInfo({ name: '訪客', email: loginEmail });
            setRole('student');
            setShowLoginModal(false);
            setActiveTab('competitions');
        }
      ...
      alert('登入失敗：' + error.message + '\n(請確認帳號密碼是否正確，或已在 Firebase Console 建立使用者)');
    }
  };
  ...
  // [Fix 5.3] 教練設定學生 Email (綁定帳號用)
  const handleUpdateStudentEmail = async (student) => {
    const currentVal = student.email || "";
    const newVal = prompt(`請輸入 ${student.name} 的登入 Email (需與 Authentication 一致):`, currentVal);
            ...
            <div className="space-y-6">
              ...
              {loginTab === 'student' ? (
                  ...
                  <div className="space-y-3 font-bold animate-in fade-in slide-in-from-left-4 duration-300">
                    ...
                    <button onClick={() => handleLogin()} className="w-full bg-slate-900 hover:bg-slate-800 text-white py-5 rounded-[2rem] font-black text-xl shadow-xl shadow-slate-200 transition-all active:scale-[0.98]">
                      管理員登入
                    </button>
                  </div>
              )}
                   ...
                   {filteredStudents.sort((a,b)=>a.class.localeCompare(b.class)).map(s => (
                     <div key={s.id} className="p-8 bg-white border border-slate-100 rounded-[3rem] shadow-sm hover:shadow-xl hover:shadow-slate-100 transition-all flex flex-col items-center group relative">
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-50 w-full flex justify-center gap-3">
                           {/* [Fix 5.3] 綁定學生 Email */}
                           <button 
                             onClick={() => handleUpdateStudentEmail(s)}
