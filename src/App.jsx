...
export default function App() {
  ...
  const handleLogin = async () => {
    try {
      ...
      if (loginTab === 'admin') {
         ...
         // 透過 Class + No 尋找學生資料
         // 這裡我們先從登入後的 students 狀態找 (因為 Firestore listener 需要 user 登入後才啟動)
         // 這是一個兩難：不登入拿不到 students，登入後才能拿。
         // 解決：登入成功代表 Authentication 過了。我們進入系統，顯示 Loading，等 useEffect 抓到 students 後再匹配。
      ...
      alert('登入失敗：\n' + (error.code === 'auth/invalid-credential' ? '帳號或密碼錯誤' : error.message));
    }
  };
  ...
  const handleSetStudentPassword = async (student) => {
      ...
      try {
          ...
          // 清理 secondary app (雖然 firebase js sdk 不支援直接 deleteApp in some versions, 但 local var 會被回收)
          
          alert(`✅ 成功建立帳號！\n\n登入班別: ${student.class}\n登入班號: ${student.classNo}\n密碼: ${newPassword}`);
      } catch (error) {
          if (error.code === 'auth/email-already-in-use') {
              alert("⚠️ 帳號已存在。\n\n如需重置密碼，請該學生自行聯絡教練或使用忘記密碼功能 (需 Email 接收)。\n(或是教練需在 Firebase Console 手動重置/刪除)");
          } else {
                   ...
                   {filteredStudents.sort((a,b)=>a.class.localeCompare(b.class)).map(s => (
                     <div key={s.id} className="p-8 bg-white border border-slate-100 rounded-[3rem] shadow-sm hover:shadow-xl hover:shadow-slate-100 transition-all flex flex-col items-center group relative">
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-50 w-full flex justify-center gap-3">
                           {/* [Fix 5.4] 帳號管理按鈕 (Key) - New Feature */}
                           <button 
